...
I was able to change the value but when I tried to click the button labeled `Respec` it wouldn't do anything. I again inspected using Dev Tools andfound this had `disabled` and `cursor-not-allowed` modifiers. 
...
```
This caused the `yes` popup to appear, but still didn't do anything. Off to the page source code! In looking at the source I found a werid line about a `DEFAULT_COOKIE`.
```javascript
 const ABCDEFG = 73;
...
WIS: 55
```
It returned the starting values! If we can modify the `FLAG` value to 100, we should get the flag! We can just do the oposite of the decoding we just did!
...
// Converts the array of encrypted numbers back to a string
const raw = String.fromCharCode(...bytes);

// btoa() encodes the string as base64
// This gives you the final encrypted cookie value
const encoded = btoa(raw);

// Sets the browser cookie with your new encrypted stats
